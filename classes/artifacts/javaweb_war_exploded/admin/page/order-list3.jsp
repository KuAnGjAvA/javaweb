...
<%--  </header>--%>
  ...
  <%--                        <li><a id="aaa" value="aa" href="order-list1.jsp">所有订单5</a></li>--%>
  <%--                        <li><a href="#">待处理</a></li>--%>
  <%--                        <li><a href="#">处理中</a></li>--%>
  <%--                        <li><a href="#">已发货</a></li>--%>
  <%--                        <li><a href="#">已交付</a></li>--%>
  <%--                    </ul>--%>
  <%--                    <% request.setAttribute("lzk", "abc"); %>--%>
  <%--                </li>--%>
  <%--                <li class="treeview">--%>
...
<script>
  ...
  $(function () {
    $('#my_order').DataTable({
      ...
      'ajax': '../action/AllOrderServlet?sta=处理中',
      'columns': [
        {'data': 'orderId'},// 订单编号219
        {
          'data': 'sta', 'render': function (data, type, row, meta) {
            ...
            switch (meta.row % 4) {
              case 0:
                content = '<span class="label bg-yellow">' + data + '</span>';
                break;
              case 1:
                content = '<span class="label bg-aqua">' + data + '</span>';
                break;
              case 2:
                content = '<span class="label bg-green">' + data + '</span>';
                break;
              case 3:
                content = '<span class="label bg-red">' + data + '</span>';
                break;
            }
            return content;
          }
        ...
        {
          'data': 'sta', 'render': function (data, type, row, meta) {
            var content = '<a href="../action/OrderDetailServlet?orderId=' + row.orderId + '">查看</a>丨';
            if (data == '已交付') {
              content += '处理';
            } else {
              content += '<a href="../action/OrderProcessServlet?orderId=' + row.orderId + '">处理</a>';
            }
            return content;
          }
